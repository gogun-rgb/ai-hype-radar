...
| 항목 | 결과 |
| --- | --- |
| Vitest 단위/통합 테스트 | 11 files, 29 tests passed |
| Playwright E2E | desktop/mobile 2 tests passed |
| Coverage | Statements 83.2%, Branches 61.33%, Functions 79.02%, Lines 83.44% |
| CI | GitHub Actions에서 typecheck, lint, test, coverage, build, E2E 실행 |
...
Demo Mode와 API 키 없는 실행:

- `DEMO_MODE=true`이면 외부 API 실패 시 명시적으로 demo fallback을 사용합니다.
- `E2E_DEMO=true`는 Playwright 검증에서 실제 외부 API 호출을 피하기 위한 테스트 전용 설정입니다.
- Supabase가 없으면 분석 결과는 로컬 `.ai-hype-radar/analyses.json`에 저장됩니다.
- 로컬 파일 저장소는 개발과 데모용이며, 다중 사용자 production 저장소로 간주하지 않습니다.
...
| 점수 | 높을 때 의미 | 낮을 때 의미 |
| --- | --- | --- |
| Hype Score | 저장소 관심도와 최근 활동이 높음 | 공개 반응이나 활동 신호가 약함 |
| Reality Score | 문서, 유지보수, 실제 사용 근거가 강함 | 실사용 근거가 부족하거나 검증이 더 필요함 |
| Risk Score | 설치, 비용, 보안, 종속성 위험 신호가 많음 | 공개 근거상 위험 신호가 적음 |

Confidence Level은 점수의 품질을 설명합니다. 데이터가 부족하면 점수 자체를 과장하지 않고 Data Coverage와 함께 신뢰도를 낮춥니다.
...
| 명령 | 목적 |
| --- | --- |
| `npm run typecheck` | Next.js route type 생성과 TypeScript 검사 |
| `npm run lint` | ESLint 검사 |
| `npm run test` | Vitest 단위/통합 테스트 |
| `npm run test:coverage` | Coverage 측정과 threshold 검증 |
| `npm run build` | Production build 검증 |
| `npm run test:e2e` | Playwright desktop/mobile E2E |
| `npm run check` | typecheck, lint, test, build 묶음 |

GitHub Actions CI도 typecheck, lint, test, coverage, build, E2E를 실행합니다. CI는 배포하지 않고 검증만 수행합니다.
...
- 외부 API 호출은 timeout을 사용하고 실패 시 가능한 경우 명시적인 fallback으로 전환합니다.
- 자세한 신고 절차와 분석 한계는 [SECURITY.md](SECURITY.md)를 참고하세요.

## 유지보수 문서

- [CONTRIBUTING.md](CONTRIBUTING.md): 로컬 설정, 테스트 기대치, PR 가이드
- [CHANGELOG.md](CHANGELOG.md): 릴리스 변경 내역
- [SECURITY.md](SECURITY.md): 취약점 신고와 정적 분석 한계
- [tests/README.md](tests/README.md): 테스트 위치와 실행 명령
